...
output0='hydra.run.dir=./outputs/text_cross_validation/0_ver2.1/'
...
save0='trainer.save_dir=../../../ckpt/text/cv0_ver2.1'
...
cf='--cfg job'
ckpt='trainer.ckpt=../../../ckpt/text/dc_text_v3.pt'

#python src/train.py +kay=cross_text_ver2 ~data.train.path.0 ~data.val.path.0 $ckpt $save0 $c0 $output0 --cfg job

nohup python src/train.py +kay=cross_text_ver2 ~data.train.path.0 ~data.val.path.0 $ckpt $save0 $c0 $output0 > ./outputs/text_cross_validation/0_ver2.1.txt &
