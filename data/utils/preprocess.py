...
class Document:
    ...
    def to_contextual_mentions(self, max_length: int = 32) -> List[ContextualMention]:
        ...
        for m in self.mentions:
            ...
            # Context
            ctx_l, ctx_r = text[:m.start].strip(
            ).split(), text[m.end:].strip().split()
            ctx_l, ctx_r = ' '.join(
                ctx_l[-max_length:]), ' '.join(ctx_r[:max_length])
...
def reverse_term2line(t2l_path, l2t_path):
...
def parse_term_info(keyword_path):
    infos = dict()
...
def dic2docs(l2t_path, snt_path, cm_path, keyword_path):
    f = open(l2t_path, 'r', encoding='utf-8')
    g = open(cm_path, 'w', encoding='utf-8')
    line2term = json.load(f)
    f.close()
    term_infos = parse_term_info(keyword_path)
    ...
    for idx, (line_idx, terms) in enumerate(tqdm(line2term.items())):
        ...
        for term in terms:
            ...
            for key in term_info.keyword:
                ...
                if start > -1:
                    ...
                    m = Mention(cui=term_info.id, start=start, end=end,
                                text=line[start:end], types=(term_info.name == key))
                    mentions.append(m)
                    term_flag = True
                    if idx < 5:
                        print(str(m))
        ...
        if not len(mentions):
            ...
            doc = Document(id=line_idx, title='',
                           abstract=line, mentions=mentions)
...
def main():
    # reverse_term2line(t2l_path, l2t_path)
    dic2docs(l2t_path, snt_path, cm_path, keyword_path)
